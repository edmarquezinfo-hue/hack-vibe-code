...
export default function AppView() {
  ...
  const handleFavorite = async () => {
    if (!user || !app) {
      toast.error('Please sign in to favorite apps');
      return;
    }
  ...
  const handleStar = async () => {
    ...
    try {
      ...
      setApp(prev => prev ? { ...prev, stars: data.starCount } : null);
  ...
  const handleExportToGitHub = async () => {
    if (!user || !app) {
      toast.error('Please sign in to export apps');
      return;
    }

    toast.error('GitHub export coming soon!');
  };
  ...
  const handlePreviewDeploy = async () => {
    ...
    try {
      ...
      if (data.data.websocketUrl && data.data.agentId) {
        ...
        ws.onopen = () => {
          setDeploymentProgress('Connected to agent. Starting deployment...');
        ...
        ws.onerror = () => {
          setDeploymentProgress('Deployment failed. Please try again.');
  ...
  const handleSaveApp = () => {
    handlePreviewDeploy(); // For now, save triggers deployment
  };
  ...
  const handleToggleVisibility = async () => {
    ...
    try {
      ...
      toast.success(`App is now ${newVisibility === 'private' ? 'private' : 'public'}`);
    } catch (error) {
        ...
        <Card className="max-w-md">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-4">{error || 'The app you\'re looking for doesn\'t exist.'}</p>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Created {isValid(createdDate) ? formatDistanceToNow(createdDate, { addSuffix: true }) : 'recently'}</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              className={cn(
                "gap-2",
                isFavorited && "text-yellow-600 border-yellow-600"
              )}
            >
            ...
            <Button
              ...
              className={cn(
                "gap-2",
                isStarred && "text-blue-600 border-blue-600"
              )}
            >
            ...
            <Button
              ...
              className="gap-2 bg-gradient-to-r from-[#f48120] to-[#faae42] hover:from-[#faae42] hover:to-[#f48120] text-white"
            >
              <Code2 className="h-4 w-4" />
              {isOwner ? 'Continue Editing' : 'Work on this'}
            </Button>
          </div>
        </div>
              ...
              <CardContent className="p-0">
                <div className="border-t relative">
                  {appUrl ? (
                    ...
                    <div className="relative w-full h-[600px] lg:h-[800px] bg-gray-50 flex items-center justify-center">
                      {/* Frosted glass overlay */}
                      <div className="absolute inset-0 bg-white/80 backdrop-blur-sm border-2 border-dashed border-gray-300 flex items-center justify-center z-10">
                        <div className="text-center p-8">
                          <Play className="h-16 w-16 mx-auto mb-4 text-gray-400" />
                          <h3 className="text-xl font-semibold mb-2 text-gray-700">
                            App needs to be readied for preview
                          </h3>
                          <p className="text-gray-500 mb-6 max-w-md">
                            This app hasn't been saved to Cloudflare Workers yet. Deploy it to see a live preview.
                          </p>
                          {deploymentProgress && (
          ...
          <TabsContent value="code" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {app.generatedCode?.length || 0} files generated
                </CardDescription>
              </CardHeader>
              <CardContent>
                {app.generatedCode && app.generatedCode.length > 0 ? (
                  <div className="space-y-4">
                    {app.generatedCode.map((file, index) => (
                      <div key={index} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <code className="text-sm font-mono">{file.file_path}</code>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => {
                              navigator.clipboard.writeText(file.file_contents);
                        ...
                        <pre className="bg-muted p-3 rounded overflow-x-auto">
                          <code className="text-xs">{file.file_contents.slice(0, 500)}...</code>
                        </pre>
                      </div>
                    ))}
                  </div>
