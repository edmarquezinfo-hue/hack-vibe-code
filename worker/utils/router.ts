...
export class Router {
    ...
    async handle(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        try {
            ...
            if (!match) {
                logger.warn(`No route found for ${request.method} ${new URL(request.url).pathname}`);
            ...
            if (!route.methods.includes(request.method.toUpperCase())) {
                logger.warn(`Method ${request.method} not allowed for ${new URL(request.url).pathname}`);
                return methodNotAllowedResponse(route.methods);
            }

            logger.info(`Matched route: ${request.method} ${route.path}`);
