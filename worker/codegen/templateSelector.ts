...
import { TemplateListResponse} from './runnerServiceTypes';
import z from 'zod';
 ...
 */
export async function selectTemplate({ env, query, availableTemplates }: SelectTemplateArgs): Promise<TemplateSelection> {
    if (availableTemplates.length === 0) {
        console.log("No templates available for selection.");
        return { selectedTemplateName: null, reasoning: "No templates were available to choose from.", useCase: null, complexity: null, styleSelection: null };
    }

    try {
        console.log("Asking AI to select a template for query:", query, "from available templates:", availableTemplates.map(t => t.name).join(', '));
...
For style selection pick a style that suits the user query, be creative and bold and random, do not just choose the most easy option but do not pick a style that does not work at all.
Options: Minimalist Design, Brutalism, Retro, Illustrative, Kid_Playful

For your selection, provide brief but precise reasoning for why it's a match`;
...
Which template (if any) is the most suitable starting point for this query?`;
        ...
        console.log(`AI template selection result: ${selection.selectedTemplateName || 'None'}, Reasoning: ${selection.reasoning}`);
        return selection;

    } catch (error) {
        console.error("Error during AI template selection:", error);
        // Fallback to no template selection in case of error
        return { selectedTemplateName: null, reasoning: "An error occurred during the template selection process.", useCase: null, complexity: null, styleSelection: null };
    }
}