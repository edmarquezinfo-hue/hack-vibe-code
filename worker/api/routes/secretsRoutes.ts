...
export function setupSecretsRoutes(router: Router): void {
    ...
    router.get('/api/secrets/all', secretsController.getAllSecrets.bind(secretsController));
    router.post('/api/secrets', secretsController.storeSecret.bind(secretsController));
    router.patch('/api/secrets/:secretId/toggle', secretsController.toggleSecret.bind(secretsController));
