...
<GOAL>
    If there are runtime errors or bugs, review and find out the best solution to mitigate them, else perform a thorough review of the entire provided codebase (<ENTIRE CODEBASE>) against the <BLUEPRINT> and <CLIENT REQUEST>. 
...
<CONTEXT>
    ...
    •   This code review would be done in multiple iterations, so be quick and focus on the most important and urgent issues first for now.
